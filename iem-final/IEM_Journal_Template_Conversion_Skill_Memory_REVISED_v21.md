...
### 33. Tall portrait flowcharts should be split into readable column-width panels

**Mistake learned:** Simply shrinking a very tall portrait flowchart by height makes the figure unreadable. Keeping the original in a one-column section wastes the page and still causes layout problems.

Best handling:
- Detect portrait flowcharts using aspect ratio, e.g. width/height < 0.35.
- Do not place these portrait flowcharts as a single full-width one-column figure.
- Split the image vertically into readable panels with slight overlap, then insert the panels in the normal two-column flow at near full column width, around 2.5–2.6 inches.
- Preserve the original figure caption, but allow the visual figure to span several stacked panels for readability.
- Landscape or architecture diagrams can still use a temporary one-column section if their aspect ratio and content justify it.

Validation:
- Check tall figure text is readable: portrait panels should be near column width, not tiny centre-line figures.
- Confirm max panel height is reasonable and no single flowchart consumes an entire page.
- Confirm source image count is preserved conceptually, even if output inline-shape count increases because tall figures are split into panels.
